    ...
    return [
        ...
        ...(() => {
            ...
            return [
                ...
                mk('duo20',      '🎼', t('stats.medalDuo20.name'),       t('stats.medalDuo20.desc'),       jointCount >= 20,  jointSessions[19]?.saved_at,
                    t('stats.medalProgJoint', jointCount, 20),  Math.min(100, Math.round((jointCount / 20)  * 100))),
                ...
                mk('duo5h',      '🕔', t('stats.medalDuo5h.name'),       t('stats.medalDuo5h.desc'),       jointMins >= 300,
                    jointSessions.find((_, i) => Math.round(jointSessions.slice(0, i + 1).reduce((s, x) => s + effectiveSecs(x), 0) / 60) >= 300)?.saved_at,
                    t('stats.medalProgJointMins', jointMins, 300),  Math.min(100, Math.round((jointMins / 300)  * 100))),
                mk('duo50',      '💫', t('stats.medalDuo50.name'),       t('stats.medalDuo50.desc'),       jointCount >= 50,  jointSessions[49]?.saved_at,
                    t('stats.medalProgJoint', jointCount, 50),  Math.min(100, Math.round((jointCount / 50)  * 100))),
                ...
                mk('duo100',     '🏅', t('stats.medalDuo100.name'),      t('stats.medalDuo100.desc'),      jointCount >= 100, jointSessions[99]?.saved_at,
                    t('stats.medalProgJoint', jointCount, 100), Math.min(100, Math.round((jointCount / 100) * 100))),
                mk('duo25h',     '🏆', t('stats.medalDuo25h.name'),      t('stats.medalDuo25h.desc'),      jointMins >= 1500,
                    jointSessions.find((_, i) => Math.round(jointSessions.slice(0, i + 1).reduce((s, x) => s + effectiveSecs(x), 0) / 60) >= 1500)?.saved_at,
                    t('stats.medalProgJointMins', jointMins, 1500), Math.min(100, Math.round((jointMins / 1500) * 100))),
                mk('duo50h',     '🥇', t('stats.medalDuo50h.name'),      t('stats.medalDuo50h.desc'),      jointMins >= 3000,
                    jointSessions.find((_, i) => Math.round(jointSessions.slice(0, i + 1).reduce((s, x) => s + effectiveSecs(x), 0) / 60) >= 3000)?.saved_at,
                    t('stats.medalProgJointMins', jointMins, 3000), Math.min(100, Math.round((jointMins / 3000) * 100))),
            ];
        })(),
    ];
}
